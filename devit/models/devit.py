"""
Деформационно-эквивариантный визуальный трансформер (DE-ViT).
...
- Иерархический backbone на базе визуального трансформера с деформируемым вниманием
- Четыре параллельные параметрические головки (классификация, регрессия рамок,
  оценка солнечного азимута, семантическая сегментация типов поверхности крыши)
- Модуль геометрии крыши
- Блок попарного связывания (Pairwise Link MLP) для предсказания вероятности связности фрагментов
- Деформационно-эквивариантный детектор с раздельными полями деформации и перекрёстным вниманием
"""
...
class DeformableAttention(nn.Module):
    """
    Деформируемое внимание с предсказанием масштабируемого поля смещения.
    
    Для каждого запроса q_i предсказывается масштабируемое поле смещения:
    Δ_ij = s * tanh(W_offset * q_i + b_offset) ∈ ℝ²
    
    Деформированная позиция для ключа/значения:
    p_{j|i}^{(def)} = p_j + Δ_ij
    
    Используется для билинейного семплирования признаков, что обеспечивает субпиксельную точность:
    k̃_j = Bilinear(K, p_{j|i}^{(def)}), ṽ_j = Bilinear(V, p_{j|i}^{(def)})
    
    Итоговый вес внимания вычисляется в деформированных координатах с учётом 
    относительного позиционного кодирования:
    α_ij = Softmax((q_i · k̃_j) / √d + PE(p_{j|i}^{(def)} - p_i))
    """
...
class DualDeformableAttention(nn.Module):
    ...
    Для каждого класса c ∈ {tree, shadow} предсказывается независимое поле смещения,
    имеющее разный априорный масштаб:
    
    Δ_c(p) = s_c * tanh(W_offset^(c) * f(p) + b_offset^(c))
    
    где s_tree = 0.15 (плавные деформации кроны, не более 15% от размера),
          s_shadow = 0.45 (изломы тени на коньках достигают 40-45%)
    
    Отношение s_shadow / s_tree = 3 отражает различную природу деформаций:
    - кроны деформируются плавно (ветер, обрезка)
    - тени могут иметь резкие изломы на коньках крыш
    
    Для теней поле деформации дополнительно модулируется типом поверхности R(p):
    Δ_shadow(p) = Δ_base(p) + W_roof * embed(R(p))
    
    где R(p) ∈ {0, 1, 2, 3}: грунт, плоская крыша, скат, конёк
    """
...
class CrossAttentionWithSun(nn.Module):
    ...
    Управляемый механизм перекрёстного внимания использует оценку направления на солнце:
    
    α_ij^cross = Softmax((q_i^tree · k_j^shadow) / √d 
                         + PE(p_j^shadow - p_i^tree) 
                         + PE(θ_sun))
    
    Компонент PE(θ_sun) позволяет механизму внимания учитывать глобальный контекст 
    освещения сцены, что значительно упрощает задачу поиска соответствий.
    
    В отличие от стандартного кросс-внимания в DETR, которое использует только 
    визуальные признаки, предложенный механизм вводит геометрический prior, 
    основанный на модели солнечной проекции.
    """
...
class HierarchicalBackbone(nn.Module):
    ...
    Архитектура аналогична SegFormer с внедрением деформируемого внимания на стадиях 
    с высоким семантическим разрешением.
    
    Этапы backbone:
    1. H/4 x W/4 x 64, 2 блока, жёсткое оконное внимание (без деформации)
       - Инициализация иерархической обработки признаков backbone
    2. H/8 x W/8 x 128, 2 блока, локальное деформируемое внимание, Δ_tree
       - Понижение пространственного разрешения до H/8 с увеличением глубины признаков
       - Формирование базового поля деформации для крон
    3. H/16 x W/16 x 256, 4 блока, полное деформируемое внимание, Δ_tree, Δ_shadow
       - Формирование семантической информации, достаточной для дифференциации 
         и предсказания деформаций кроны
       - Разделение на независимые поля Δ_tree и Δ_shadow
    4. H/32 x W/32 x 512, 2 блока, глобальное деформируемое внимание, Δ_tree, Δ_shadow
       - Построение глобального семантического контекста для декодера и головок
    """
...
class RoofGeometryModule(nn.Module):
    ...
    Выполняет семантическую сегментацию типов поверхности крыши:
    0 - грунт
    1 - плоская крыша
    2 - скат
    3 - конёк
    
    Выходная карта R(p) используется для:
    - Прогнозирования разрывов и изломов тени на скатных крышах
    - Модуляции поля деформации тени Δ_shadow вблизи коньков через 
      W_roof · embed(R(p))
    """
...
class DetectionHead(nn.Module):
    ...
    Предсказывает:
    - Классы объектов (кроны, тени) с использованием фокальной потери
    - Координаты ориентированных рамок (x, y, w, h, φ)
    
    Выходные данные формируют результат детекции: множество объектов {(b_i, c_i)}
    """
...
class SunAzimuthHead(nn.Module):
    ...
    Предсказывает азимут θ_sun для моделирования геометрии освещения.
    Выходное значение используется для:
    - Введения априорного геометрического ограничения PE(θ_sun) в перекрёстном внимании
    - Обеспечения коллинеарности вектора «крона-тень» азимуту без требования знания высоты дерева
    
    Выход: sin/cos представление азимута для непрерывного кодирования направления.
    """
...
class ShadowConnectionHead(nn.Module):
    """
    Головка классификации связности пар фрагментов теней (Pairwise Link MLP).
    
    Вычисляет вероятность связности p_ij на основе конкатенации признаков запросов:
    p_ij = MLP([q_i, q_j])
    
    Если два фрагмента являются частями одной тени, разорванной коньком, 
    они получают метку y_ij = 1.
    
    Потеря имеет вид бинарной кросс-энтропии:
    L_connect = -Σ_{(i,j) ∈ frags} [y_ij * log(p_ij) + (1 - y_ij) * log(1 - p_ij)]
    
    Данная потеря побуждает модель воспринимать разорванные коньком фрагменты 
    как единый объект, что критически важно для корректной детекции теней на скатных крышах.
    
    При p_ij > 0.5 фрагменты объединяются в единый объект shadow с общей рамкой.
    """
...
class DEViT(nn.Module):
    """
    Deformation-Equivariant Visual Transformer (DE-ViT)
    
    Полная архитектура включает:
    - Иерархический backbone с деформируемым вниманием
    - Четыре параллельные параметрические головки:
      * Классификация объектов (кроны, тени) с фокальной потерей
      * Регрессия ориентированных ограничивающих рамок (x, y, w, h, φ)
      * Оценка солнечного азимута θ_sun для моделирования геометрии освещения
      * Семантическая сегментация типов поверхности крыши (грунт, плоская, скат, конёк)
    - Модуль геометрии крыши для прогнозирования разрывов и изломов тени
    - Блок попарного связывания (Pairwise Link MLP) для предсказания вероятности связности p_ij
    - Деформационно-эквивариантный детектор с раздельными полями деформации и перекрёстным вниманием
    
    Ключевое свойство: приближённая локальная деформационная эквивариантность.
    Небольшая деформация входного снимка приводит к согласованной деформации предсказаний 
    с точностью до ошибки интерполяции.
    """

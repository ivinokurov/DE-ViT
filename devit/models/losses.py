...
L_total = L_det + λ₁ * L_def + λ₂ * L_cross + λ₃ * L_roof + λ₄ * L_connect

где:
- λ₁ = 0.05 (регуляризация деформаций)
- λ₂ = 0.5 (согласованность крона-тень)
- λ₃ = 0.2 (согласованность с геометрией крыши)
- λ₄ = 0.3 (связывание фрагментов теней)

Гиперпараметры подобраны на валидационной выборке.

Детекционная потеря состоит из трёх компонентов:
L_det = L_cls + L_box + L_giou

где:
- L_cls -- фокальная потеря (Focal Loss) для классификации объектов
- L_box -- L1-потеря для регрессии координат ограничивающих рамок (x, y, w, h, φ)
- L_giou -- обобщённая IoU-потеря (Generalized Intersection over Union)
"""
...
class DeformationSmoothnessLoss(nn.Module):
    ...
    Обеспечивает физически осмысленную плавность деформаций крон и предотвращает артефакты.
    Штрафует большие значения градиента поля смещения:
    
    L_def = (1/N) * Σ_i (||∇Δ_tree(p_i)||_F² + ||∇Δ_shadow(p_i)||_F²)
    
    где ||·||_F -- норма Фробениуса, применяемая к матрице Якоби 
    двумерного поля смещения.
    """
...
class CrossConsistencyLoss(nn.Module):
    ...
    Явно связывает геометрические центры кроны и её тени, используя априорную 
    информацию о положении солнца:
    
    L_cross = Σ_{i,j} ||(p_j^shadow - p_i^tree) / ||p_j^shadow - p_i^tree|| + ε - u(θ_sun)||² · I_{(i,j) ∈ pairs}
    
    где:
    - u(θ_sun) = (cos θ_sun, sin θ_sun)^T -- единичный вектор направления проекции 
      солнечных лучей на горизонтальную плоскость
    - ε -- малая константа для предотвращения деления на ноль
    - I_{(i,j) ∈ pairs} -- бинарный индикатор, активирующий потерю только для 
      сопоставленных пар «крона-тень»
    
    В отличие от исходной физической модели проекции, используется ослабленное 
    геометрическое ограничение, не требующее знания высоты дерева h_tree.
    Штрафуется исключительно отклонение направления вектора смещения от азимута солнца 
    (коллинеарность), однако не фиксируется его длина.
    """
...
class RoofConsistencyLoss(nn.Module):
    ...
    Обеспечивает физически осмысленные предсказания деформаций на структурных линиях 
    и формулируется как двусторонняя регуляризация:
    
    L_roof = Σ_{p ∈ ridge} M_shadow(p) · [max(0, τ_min - ||∇Δ_shadow(p)||_F) + max(0, ||∇Δ_shadow(p)||_F - τ_max)]
    
    где:
    - τ_min = 0.3 и τ_max = 1.5 -- пороги градиента
    - M_shadow(p) ∈ {0, 1} -- бинарная маска наличия тени, гарантирующая применение 
      штрафа только к покрытым тенью пикселям конька
    
    Данная формулировка штрафует как слишком малые (отсутствие излома), так и 
    слишком большие (нефизичная деформация) градиенты на коньках, предотвращая 
    ложноположительные срабатывания.
    
    При вычислении матрицы Якоби (градиента ∇) карта типов поверхности R(p) 
    рассматривается как кусочно-постоянный модулятор (градиент берётся только от 
    непрерывной компоненты Δ_base), что позволяет избежать математических сингулярностей 
    (дельта-функций) на границах классов сегментации.
    """
...
class ShadowConnectionLoss(nn.Module):
    ...
    Решает задачу классификации связности пар фрагментов теней. Вероятность связности 
    p_ij вычисляется легковесным многослойным перцептроном (MLP), принимающим на вход 
    конкатенацию признаков запросов q_i и q_j на выходе декодера.
...
class DEViTLoss(nn.Module):
    ...
    где:
    - L_det = L_cls + L_box + L_giou -- детекционная потеря
      * L_cls -- фокальная потеря для классификации объектов
      * L_box -- L1-потеря для регрессии координат рамок (x, y, w, h, φ)
      * L_giou -- обобщённая IoU-потеря
    - L_def -- регуляризация плавности деформаций
    - L_cross -- согласованность «крона-тень» с использованием азимута солнца
    - L_roof -- согласованность с геометрией крыши (двусторонняя регуляризация на коньках)
    - L_connect -- связывание фрагментов теней, разорванных коньком
    """
